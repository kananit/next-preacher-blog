...
// strip content from post objects for reducing page data size
export const stripContent = (posts) => {
  return posts.map(({ frontmatter, slug }) => ({
    frontmatter,
    slug,
  }));
};

// strip content from a single post object
export const stripContentItem = ({ frontmatter, slug }) => ({
  frontmatter,
  slug,
});
