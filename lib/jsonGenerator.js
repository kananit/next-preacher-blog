...
// Same slugify as lib/utils/textConverter.js (github-slugger) so slugs in the
// search index match the URLs actually served by the Next.js pages.
...
sectionIds.forEach((sectionId) => {
  ...
  const posts = allMdFiles.map((filename) => {
    ...
    const postData = fs.readFileSync(path.join(sectionPath, filename), "utf-8");
    ...
    const firstNonEmptyIdx = contentLines.findIndex(
      (line) => line.trim() !== "",
    );
    if (
      firstNonEmptyIdx !== -1 &&
